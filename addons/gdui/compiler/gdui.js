...
function compileThemeFile(inputFile, outputFile, options = {}) {
  const source = import_node_fs.default.readFileSync(inputFile, "utf8");
  const themeTokens = JSON.parse(source);
  const result = exportTheme(themeTokens, options);
  if (outputFile) {
    import_node_fs.default.mkdirSync(import_node_path.default.dirname(outputFile), { recursive: true });
    import_node_fs.default.writeFileSync(outputFile, result.content, "utf8");
  }
  return result;
}
...
function compileFile(inputFile, outputFile, options = {}) {
  const source = import_node_fs2.default.readFileSync(inputFile, "utf8");
  const result = compileSource(source, options);
  if (outputFile) {
    import_node_fs2.default.mkdirSync(import_node_path2.default.dirname(outputFile), { recursive: true });
    import_node_fs2.default.writeFileSync(outputFile, result.tscn, "utf8");
    if (options.genScript) {
      const scriptResult = generateActionScript(result.sceneAst, {
        sourceName: import_node_path2.default.basename(inputFile)
      });
      const scriptFile = outputFile.replace(/\.tscn$/i, "Actions.gd");
      import_node_fs2.default.writeFileSync(scriptFile, scriptResult.content, "utf8");
...
function compileDirectory(inputDir, outputDir, options = {}) {
  ...
  for (const file of files) {
    const source = import_node_fs2.default.readFileSync(file, "utf8");
    const result = compileSource(source, options);
    const baseName = `${result.sceneAst.name}.tscn`;
    const outFile = import_node_path2.default.join(outputDir, baseName);
    import_node_fs2.default.mkdirSync(import_node_path2.default.dirname(outFile), { recursive: true });
    import_node_fs2.default.writeFileSync(outFile, result.tscn, "utf8");
    if (options.genScript) {
      const scriptResult = generateActionScript(result.sceneAst, {
        sourceName: import_node_path2.default.basename(file)
      });
...
async function runCli(argv = process.argv.slice(2)) {
  ...
  const theme = args.theme ? import_node_path2.default.resolve(cwd, args.theme) : null;
  const input = import_node_path2.default.resolve(cwd, args.input || args.i || "ui");
  const output = args.output || args.o ? import_node_path2.default.resolve(cwd, args.output || args.o) : null;
  const check = Boolean(args.check);
  const genScript = Boolean(args["gen-script"]);
  if (theme) {
    if (!import_node_fs2.default.existsSync(theme)) {
      console.error(`[gdui] Theme input not found: ${theme}`);
      process.exitCode = 1;
      return;
    }
    try {
      const outFile = output || import_node_path2.default.resolve(cwd, "scenes", "theme.tres");
      ...
      console.log(
        `[gdui] ${import_node_path2.default.relative(cwd, theme)} -> ${import_node_path2.default.relative(cwd, outFile)}`
      );
    } catch (error) {
  ...
  if (!import_node_fs2.default.existsSync(input)) {
    console.error(`[gdui] Input not found: ${input}`);
    process.exitCode = 1;
    return;
  }
  try {
    if (import_node_fs2.default.statSync(input).isDirectory()) {
      const outDir = output || import_node_path2.default.resolve(cwd, "scenes");
      const results = compileDirectory(input, outDir, { ...args, genScript });
      for (const item of results) {
        printWarnings(item.warnings);
        console.log(
          `[gdui] ${import_node_path2.default.relative(cwd, item.input)} -> ${import_node_path2.default.relative(cwd, item.output)}`
        );
        if (item.scriptFile)
          console.log(
            `[gdui] script -> ${import_node_path2.default.relative(cwd, item.scriptFile)} (${item.actions?.length ?? 0} actions)`
          );
      }
      if (!results.length)
    ...
    const outFile = output || import_node_path2.default.resolve(
      cwd,
      "scenes",
      import_node_path2.default.basename(input).replace(/\.gdui\.html$/i, ".tscn")
    );
    ...
    console.log(
      `[gdui] ${import_node_path2.default.relative(cwd, input)} -> ${import_node_path2.default.relative(cwd, outFile)}`
    );
    if (result.scriptFile)
      console.log(
        `[gdui] script -> ${import_node_path2.default.relative(cwd, result.scriptFile)} (${result.actions?.length ?? 0} actions)`
      );
  } catch (error) {
    console.error(`[gdui] ${error.message}`);
    if (args.debug) console.error(error.stack);
